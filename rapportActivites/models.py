...
class RapportActivite(models.Model):
    ...
    # Travaux réalisés
    plainte_client =models.CharField(max_length=50,verbose_name='Plainte du client',blank=True)
    quel_brain =models.CharField(max_length=50,verbose_name="Brains de raccordement",blank=True)
    tube = models.CharField(max_length=20,verbose_name="Tube pour le raccordement",blank=True)
    quel_joinbox= models.CharField(max_length=50,verbose_name="Lequel des JoinBox pour le raccordement",blank=True)
    type_de_brain = models.CharField(max_length=20,verbose_name="Type de brain utilisé",blank=True)
    point_de_depart = models.TextField(verbose_name="Point_de_part",blank=True)
    nombre_de_joinbox_poser = models.TextField(verbose_name="Nombre de joinbox poser",blank=True)
    description = models.TextField(verbose_name="Desption",blank=True)
    ...
    photo_appareils_connecte = models.ImageField(upload_to='rapports/photos/appareilsconnecte/', null=True, blank=True,
                                    verbose_name="Photo appareils connectés")
    photo_ping = models.ImageField(upload_to='rapports/photos/ping/', null=True, blank=True,
                                    verbose_name="Photo du ping")
    ...
    def __str__(self):
        return f"Rapport {self.activite.get_type_activite_display()} - {self.activite.client.nom_client} - {self.date_intervention_reelle}"
